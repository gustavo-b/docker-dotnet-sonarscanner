...
PROJECT_ORGANIZATION="${PROJECT_ORGANIZATION:-default}"
...
mono /opt/sonar-scanner-msbuild/SonarScanner.MSBuild.exe begin /d:sonar.host.url=$SONAR_HOST /d:sonar.login=$SONAR_LOGIN_KEY /k:$PROJECT_KEY /o:"$PROJECT_ORGANIZATION" /n:"$PROJECT_NAME" /v:$PROJECT_VERSION
dotnet restore
dotnet build
dotnet test
